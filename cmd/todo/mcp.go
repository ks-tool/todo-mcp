...
func runMCP(st *todo.Store) error {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "contract",
		Description: "Check an API contract between two services from their specs — OpenAPI (JSON/YAML), AsyncAPI, a gRPC .proto, or a GraphQL SDL, chosen by file format. consumer is the spec the caller was built against, provider the spec the service now offers. Returns the endpoints present and compatible in both, and the breaks: orphan-call (an endpoint the provider dropped) and schema-drift (a request/response shape that diverged)."},
